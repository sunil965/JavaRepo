...
public class AddressBook implements Service {
	...
	public void addPerson() {
		...
		if (numberofaddressbook == 0) {
			System.out.println("Address Book Not Available, Press 1 to Create First ");
			...
			System.out.println("Select Your Address Book By Their Index");
			int select = scan.nextInt();
			bookname = addressbook[select];
		}
		...
		if (hashObj.get(bookname) == null) {
			hashObj.put(bookname, list);
		}

		/*List<Person> newlist = hashObj.get(bookname);
		newlist.add(person);
		hashObj.put(bookname, newlist);*/
	}
	...
	public void printList() {
		...
		for (int getkey = 0; getkey < numberofaddressbook; getkey++) {
			System.out.println(getkey + " " + addressbook[getkey]);
		}

		System.out.println("Please Select the Key");
		int selected_key = scan.nextInt();
		String j = addressbook[selected_key];

		List<Person> a = hashObj.get(j);
	...
	public void createNewAddressBook() {
		...
		addressbook[numberofaddressbook] = scan.next();

		hashObj.put(addressbook[numberofaddressbook], new LinkedList<Person>());
		numberofaddressbook++;
